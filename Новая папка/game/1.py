...
# Создание таблицы "snakegame", если её ещё нет
...
# Функция для получения уровня пользователя из базы данных
...
# Функция для получения всех данных из базы данных (таблица лидеров)
def get_all_data():
...
# Функция для получения счета пользователя из базы данных
...
# Функция для получения никнейма пользователя с использованием Pygame
def get_nickname():
...
if cur.fetchone()[0]==0:
     # Если пользователь не существует, сохраняем его состояние игры с начальными значениями
    save_game_state(NICKNAME, 0, 0)
    conn.commit()
else:
     # Если пользователь существует, извлекаем его данные из базы данных
...
# Отображение таблицы лидеров перед запуском главного цикла игры
get_all_data()
